...
# Comment by Maali Tars: modified the original file from https://github.com/macsnoeren/python-p2p-network after what was shown in seminar 8 and my own additions
...
class Node(threading.Thread):
    ...
    def __init__(self, port, id=None, state='DO-NOT-WANT', timeout=5, access_granted=False, callback=None, resource_timeout=10, max_connections=0):
        """Create instance of a Node. If you want to implement the Node functionality with a callback, you should 
           provide a callback method. It is preferred to implement a new node by extending this Node class. 
            host: The host name or ip address that is used to bind the TCP/IP server to.
            port: The port number that is used to bind the TCP/IP server to.
    ...
    def node_callback(self, event, connected_node, data):
        if event == 'lookup':
            endNode, steps = data.split(';')
            self.search(int(endNode), int(steps))
        elif event == 'connect':
            self.connect_with_successor(int(data))
            self.get_next_successor()
        elif event == 'joinNode':
            newNode, newPort = data.split(';')
            self.joinNode(int(newNode), int(newPort))
        elif event == 'update_next_successors':
            start = int(data)
            if self.id != start:
                self.get_next_successor()
                self.successor.send(('update next successors:%s' % (data)).encode('utf-8'))
        elif event == 'send_successor':
            connected_node.send(('receive next successor:%d' % (self.successor.id)).encode('utf-8'))
            ...
            connected_node.send(('connected:%d' % (self.successor.port)).encode('utf-8'))
            self.stop()
    ...
    def debug_print(self, message):
        ...
        if self.debug:
            print("DEBUG (" + str(self.id) + "): " + message)
    ...
    def run(self):
        ...
        while not self.terminate_flag.is_set():  # Check whether the thread needs to be closed
            ...
            if self.state == 'DO-NOT-WANT':
                time.sleep(random.randint(5, int(self.timeout) + 1))
                self.state = 'WANTED'
                self.request_queue.append(self.id)
                self.send_to_nodes('WANTED ' + str(self.id))
            if self.access_granted and self.state=='WANTED':
                self.state = 'HELD'
                self.request_queue.pop()
                time.sleep(random.randint(10, int(self.resource_timeout) + 1))
                access_deployed = False
                next_node = self.next_successor
                while not access_deployed:
                    if next_node.state == 'WANTED':
                        next_node.access_granted = True
                        access_deployed = True
                    else:
                        next_node = next_node.next_successor
    ...
    def node_message(self, node, data):
        ...
        if 'WANTED' in data:
            if self.state == 'WANTED' or self.state == 'DO-NOT-WANT':
                #self.callback('update_next_successors', node, 'ok')
                self.next_successor.access_granted = True
        #if self.callback is not None:
        #        self.callback('node_message', self, node, 'ok')
        #    elif self.state == 'HELD':
        #        self.callback('node_message', self, node, 'not ok')
    ...
    def search(self, endNode, steps=0):
        if endNode == self.id:
            if steps == 1:
                print('Data stored in node', self.id, "-", steps, "request sent")
                return
            else:
                print('Data stored in node', self.id, "-", steps, "requests sent")
                return
        nextNode = None
        foundShortcut = False
        ...
        if len(self.shortcuts) > 0:
            for shortcut in self.shortcuts[::-1]:
                if shortcut.id <= endNode:
                    foundShortcut = True
                    nextNode = shortcut
                    break

        if nextNode == None: nextNode = self.successor

        if foundShortcut:
            nextNode.send(('lookup:%d;%d' % (endNode, steps + 1)).encode('utf-8'))
        elif self.successor.id <= endNode and self.successor.id > self.id:
            nextNode.send(('lookup:%d;%d' % (endNode, steps + 1)).encode('utf-8'))
        elif self.successor.id >= endNode and self.id < endNode:
            if steps == 0:
                print('Data stored in node', self.successor.id, '-', steps + 1, 'request sent')
            else:
                print('Data stored in node', self.successor.id, '-', steps + 1, 'requests sent')
        else:
            nextNode.send(('lookup:%d;%d' % (endNode, steps + 1)).encode('utf-8'))
    ...
    def joinNode(self, newNode, newNodePort):
        if newNode > self.id and newNode < self.successor.id or (newNode > self.id and self.id >= self.successor.id) or (newNode < self.successor.id and newNode < self.id and self.id >= self.successor.id):
            oldConnection = self.successor
            self.connect_with_successor(newNodePort)
            self.successor.send(('connect:%d' % (oldConnection.port)).encode('utf-8'))

            oldConnection.stop()

            self.get_next_successor()
            self.successor.send(('update_next_successors:%d' % (self.id)).encode('utf-8'))
        else:
            self.successor.send(('joinNode:%d;%d' % (newNode, newNodePort)).encode('utf-8'))


    def leave(self, node):
        if self.successor.id == node:
            oldSuccessor = self.successor
            self.successor.send('pop'.encode('utf-8'))
            oldSuccessor.stop()
            self.successor.send(('update_next_successors:%d' % (self.id)).encode('utf-8'))
        else:
            ...
            for i in range(len(self.shortcuts)):
                ...
                if shortcut.id == node:
                    ...
                    break
            if remove_shortcut is not None:
                self.shortcuts.remove(remove_shortcut)
                remove_shortcut.stop()
            self.successor.send(('leave:%d' % (node)).encode('utf-8'))
